      ...
      <header class="p-6 border-b border-border bg-gradient-to-r from-muted/30 to-transparent">
        <div class="flex flex-col md:flex-row md:items-center justify-between gap-6">
          <div class="flex items-center gap-5">
            ...
            <div class="space-y-1.5">
              <div class="flex items-center gap-2 flex-wrap">
                ...
                <Badge variant="secondary" class="bg-emerald-500/10 text-emerald-600 border-emerald-500/20 font-bold">ACTIVA</Badge>
              </div>
              <div class="flex items-center gap-3 text-sm font-medium text-muted-foreground flex-wrap">
                ...
                <span v-if="selectedModel.location" class="flex items-center gap-1"><span class="w-1 h-1 rounded-full bg-border" /> {{ selectedModel.location }}</span>
                <span v-if="selectedModel.age" class="flex items-center gap-1"><span class="w-1 h-1 rounded-full bg-border" /> {{ selectedModel.age }} años</span>
              </div>
              
              <!-- Social Links -->
              <div class="flex items-center gap-2 pt-2">
                <Button v-if="selectedModel.instagram" variant="outline" size="sm" class="h-8 text-[10px] font-bold uppercase tracking-wider gap-1.5 px-3 rounded-full border-pink-500/20 hover:bg-pink-500/5 text-pink-600">
                  <Instagram class="w-3.5 h-3.5" /> {{ selectedModel.instagram }}
                </Button>
                <Button v-if="selectedModel.telegram" variant="outline" size="sm" class="h-8 text-[10px] font-bold uppercase tracking-wider gap-1.5 px-3 rounded-full border-sky-500/20 hover:bg-sky-500/5 text-sky-600">
          ...
          <div class="grid grid-cols-2 gap-3 shrink-0">
            <div class="p-3 bg-violet-500/5 border border-violet-500/10 rounded-xl flex flex-col items-center justify-center text-center">
            ...
            <div class="p-3 bg-amber-500/5 border border-amber-500/10 rounded-xl flex flex-col items-center justify-center text-center">
          ...
          <div class="p-6">
            <Tabs default-value="bio" class="space-y-6">
              <TabsList class="w-full justify-start h-12 p-1.5 bg-muted/50 rounded-xl">
                <TabsTrigger value="bio" class="px-6 rounded-lg text-xs font-bold uppercase tracking-wide">Bio & Personaje</TabsTrigger>
                <TabsTrigger value="body" class="px-6 rounded-lg text-xs font-bold uppercase tracking-wide">Atributos</TabsTrigger>
                <TabsTrigger value="sop" class="px-6 rounded-lg text-xs font-bold uppercase tracking-wide">SOP & Ventas</TabsTrigger>
                <TabsTrigger value="history" class="px-6 rounded-lg text-xs font-bold uppercase tracking-wide flex items-center gap-2">
              ...
              <TabsContent value="bio" class="space-y-6">
                <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                  <Card class="border-border/60 shadow-none bg-muted/10">
                    <CardHeader class="pb-3">
                      <CardTitle class="text-xs font-black uppercase tracking-widest text-primary flex items-center gap-2">
                  ...
                  <Card class="border-border/60 shadow-none col-span-1 md:col-span-2">
                    ...
                    <CardContent class="space-y-4">
                      <p class="text-sm leading-relaxed text-foreground/80 whitespace-pre-line bg-muted/20 p-4 rounded-xl italic">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div class="p-5 rounded-2xl bg-rose-500/5 border border-rose-500/10 space-y-3">
                    ...
                    <p class="text-sm font-medium text-rose-600/80 dark:text-rose-400/80 leading-relaxed">{{ selectedModel.hates || 'Sin datos.' }}</p>
                  </div>
                  <div class="p-5 rounded-2xl bg-amber-500/5 border border-amber-500/10 space-y-3">
                    ...
                    <p class="text-sm font-medium text-amber-600/80 dark:text-amber-400/80 leading-relaxed">{{ selectedModel.specialNotes || 'Sin notas.' }}</p>
                  </div>
                </div>
              </TabsContent>
              ...
              <TabsContent value="body" class="space-y-6">
                <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                  ...
                  <Card class="border-border/60 shadow-none col-span-1 md:col-span-2 flex flex-col">
                    ...
                    <CardContent class="flex-1 flex flex-col justify-center bg-muted/10 m-4 rounded-xl border border-dashed border-border/60">
                      <div class="p-6 text-center space-y-3">
                         ...
                         <p class="text-[10px] text-muted-foreground uppercase font-black tracking-widest">Crucial para verificar autenticidad en chats</p>
                      </div>
                    </CardContent>
                  </Card>
                </div>
              ...
              <TabsContent value="sop" class="space-y-6">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <Card class="border-rose-500/20 bg-rose-500/5 shadow-none">
                    <CardHeader class="pb-3">
                      <CardTitle class="text-xs font-black uppercase tracking-widest text-rose-600 dark:text-rose-400 flex items-center gap-2">
                  ...
                  <Card class="border-emerald-500/20 bg-emerald-500/5 shadow-none">
                    <CardHeader class="pb-3">
                      <CardTitle class="text-xs font-black uppercase tracking-widest text-emerald-600 dark:text-emerald-400 flex items-center gap-2">
                    ...
                    <CardContent class="space-y-4">
                      ...
                      <div>
                        <p class="text-[9px] font-black text-emerald-600 uppercase mb-1">Instrucciones de Contenido Personalizado</p>
                ...
                <Card class="border-border/60 shadow-none">
                   ...
                   <CardContent>
                      <div class="flex flex-wrap gap-2">
                        <Badge v-for="tag in contentTags" :key="tag.tag" variant="secondary" class="px-4 py-2 text-[10px] font-black uppercase tracking-wider rounded-full bg-muted/50 border-border/50">
              ...
              <TabsContent value="history" class="space-y-6">
                <div v-if="logbookEntries.length > 0" class="space-y-4">
                  <div v-for="entry in logbookEntries" :key="entry.id" class="p-4 rounded-xl border border-border bg-muted/5 space-y-3">
                    <div class="flex items-center justify-between">
                      <div class="flex items-center gap-2">
                        ...
                        <span class="text-[10px] text-muted-foreground font-medium">• {{ new Date(entry.timestamp).toLocaleString() }}</span>
                      </div>
                      <Badge v-if="entry.priority === 'HIGH'" variant="destructive" class="text-[9px] font-black">URGENTE</Badge>
                    </div>
                    <p class="text-sm text-foreground/80 leading-relaxed whitespace-pre-line">{{ entry.message }}</p>
                  </div>
                </div>
                <div v-else class="flex flex-col items-center justify-center py-20 text-center gap-4 border-2 border-dashed border-border rounded-2xl">
