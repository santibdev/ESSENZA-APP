...
const fetchAnnouncements = async () => {
  try {
    const res = await fetch(`${import.meta.env.VITE_API_BASE_URL || 'http://localhost:8080/api/v1'}/announcements`, {
...
<template>
  <div v-if="show && announcements.length > 0" class="space-y-3">
    <div v-for="ann in announcements" :key="ann.id" 
      ...
      <!-- Decor -->
      <div class="absolute top-0 right-0 p-12 opacity-10 translate-x-1/4 -translate-y-1/4 group-hover:scale-110 transition-transform">
         <component :is="getIcon(ann.type)" class="w-32 h-32" />
      </div>

      <div class="relative z-10 flex items-center justify-between gap-8">
        <div class="flex items-center gap-6">
           <div class="w-16 h-16 rounded-[2rem] bg-white/20 backdrop-blur-md flex items-center justify-center shadow-inner">
        ...
        <button @click="show = false" class="w-12 h-12 rounded-2xl bg-white/10 hover:bg-white/20 transition-all flex items-center justify-center shrink-0">
           <X class="w-6 h-6" />
        </button>
      </div>
    </div>
